...
export const SAMPLE_NEWS_LIST = [
    ...
    ::진지한 케일리::'수퍼톤 제품들의 핵심 가치는 창작 과정에서 필요한 상상력과 표현력의 경계를 확장해 창작자의 의도가 청중에게 더 깊고 울림있게 전달되는 스토리텔링을 가능하게 하는 것'
    
    이라며 
    
    ::진지한 케일리::'이를 위해 지속적으로 기술을 개발하고, 경쟁력을 강화할 것'
    ...
    ::진지한 케일리::'이번 프로젝트에서 수퍼톤의 기술은 단순히 고인의 음성 재현을 넘어, 사람들의 감정과 기억을 되살리는 중요한 역할을 했다'
    
    며,
    
    ::진지한 케일리::'앞으로 더 많은 이들에게 그리운 목소리들을 재현하는 프로젝트로 기술의 가치를 알려 나갈 것'
    ...
    ::진지한 케일리::'수퍼톤 API는 수퍼톤 AI 음성 기술들의 활용도를 넓히고 상용화하는 계기가 될 것'
    
    이라며 
    
    ::진지한 케일리::'비즈니스 파트너사 뿐만 아니라 1인 또는 소규모 콘텐츠 크리에이터들도 API 형태로 수퍼톤 AI 음성 기술들을 도입해 더 쉽고 풍성하게 서비스 제작을 할 수 있길 바란다'
    ...
    ::진지한 케일리::'이번 협업 사례는 아티스트가 어떤 물리적 한계에서도 음악을 지속할 수 있도록 돕는 기술의 순 기능을 보여준 것'
    
    이라며, 
    
    ::진지한 케일리::'방송 이후에도 더 크로스가 계속 음악 활동을 할 수 있도록 기술을 지원할 계획'
    ...
    ::진지한 케일리::'BJ 마스크걸이 기존 배역과 다른 인격을 보여주는 만큼, 제3의 목소리가 필요하다는 제작진의 연출 의도를 충족시키기 위해 수퍼톤의 기술을 활용하게 됐다'
    
    며, 
    
    ::진지한 케일리::'수퍼톤의 기술력이 콘텐츠 제작 환경의 혁신과 콘텐츠 퀄리티 향상에 기여할 수 있게 돼 뜻 깊다'
    ...
    ::진지한 케일리::'이번 MOU 체결로 K콘텐츠 내 사운드 기술의 퀀텀점프를  가져올 수 있을 것으로 기대하며, 이를 통해 혁신적이고 보다 창의적인 K-콘텐츠 제작이 가능할 것으로 예상한다'
    
    고 말했다. 이어 
    
    ::진지한 케일리::‘글로벌 영상 포스트 프로덕션 업계의 음성 기술 발전에 수퍼톤의 AI 오디오 기술을 통해 기여하고자 한다'
    ...
    ::진지한 케일리::수퍼톤이 케이팝을 시작으로 '창의적 과정의 패러다임을 변화' 시키고자 한다
    ...
    ::진지한 케일리::수퍼톤은 'Masquerade' 의 데모 버전을 듣고 Face2Voice Techonology®를 사용하여 여성 목소리를 디자인했습니다. 그 후 프로듀셔 히치하이커님은 수퍼톤의 레퍼런스와 기타 데이터를 참고하여 미드낫의 여성 목소리를 디자인하였습니다. 
    ...
    ::진지한 케일리::히치하이커님은 아티스트가 노래를 부를 때 곡의 의미와 가사에 담긴 메시지를 자신만의 고유 스타일로 표현하고 전달하려고 노력한다고 말씀하셨습니다. 아티스트는 글로벌 팬들과 소통하기 위해 여러 언어로 노래하려고 노력하는데 발음이 완벽하지 않으면 몰입도가 떨어지고 곡이 잘못 해석될 수도 있습니다. 그래서 노래의 의미를 훼손하지 않으면서 발음을 교정할 수 있는 기술을 고민했습니다. 아티스트인 미드낫 또한 이번 프로젝트가 기술과 엔터테인먼트의 첫 협업이라는 점에서 불확실성이 수반된다는 점을 잘 알고 있었지만, 팬들의 기대가 계속 높아지는 만큼 새로운 도전의 기회로 여기고 이번 프로젝트에 참여하기로 결정했습니다. 
    ...
    ::진지한 케일리::가끔 문서 작업에 있어서 GPT-4를 사용하곤 하지만 아직 이미지나 동영상과 관련된 제너레이티브 AI를 직접적으로 사용해 본 적은 없습니다. 매일 간접적으로 AI를 사용하고 있는데, 제가 유튜브에서 음악을 듣거나 동영상을 시청할때 저에게 추천되는 콘텐츠도 AI 알고리즘을 기반으로 제공되는 것이며 자동차의 반자율 주행도 또 다른 예시 입니다. 
    ...
    ::진지한 케일리::물론 그런 우려가 있을 수 있으나 다른 많은 기술과 마찬가지로 AI는 진정한 크리에이터와 아티스트의 창의력을 극대화하는 환상적인 도구로 사용될 수 있다고 확신합니다. 일렉트릭 기타의 발명을 통해 지미 헨드릭스 같은 놀라운 아티스트를 탄생했고, Daft Punk는 신시사이저를 창의적인 방법으로 활용하여 일렉트로닉 음악에 혁신을 가져왔습니다. '프로젝트 L' 공개 후 팬들의 반응을 볼때, 이번 프로젝트는 예술에 AI 기술을 접목한 긍정적인 대표 사례라고 생각합니다. 
    ...
    ::소녀::“우리, 가보지 않으련? 시골 오니까 혼자서 심심해 못 견디겠다.” 
    
    ::소년::“저래 봬도 멀다.”
    
    ::소녀::“멀면 얼마나 멀기에? 서울 있을 땐 사뭇 먼 데까지 소풍 갔었다.” 
    ...
    ::소년::“이게 들국화, 이게 싸리꽃, 이게 도라지꽃,…….”
    
    ::소녀::“도라지꽃이 이렇게 예쁜 줄은 몰랐네. 난 보랏빛이 좋아! …… 그런데, 이 양산 같이 생긴 노란 꽃이 뭐지?"
    ...
    ::소녀::“꼭 등꽃 같네. 서울 우리 학교에 큰 등나무가 있었단다. 저 꽃 을 보니까 등나무 밑에서 놀 던 동무들 생각이 난다.”
    ...
    ::소녀::“하도 갑갑해서 나왔다. ……참, 그 날 재밌었어……. 그런데 그 날 어디서 이런 물이 들었는지 잘 지지 않는다.”
    ...
    ::소녀::“내, 생각해 냈다. 그 날, 도랑을 건너면서 내가 업힌 일이 있지? 그 때, 네 등에서 옮은 물이다.”
    
    소년은 얼굴이 확 달아오름을 느꼈다.
    갈림길에서 소녀는 
    
    ::소녀::“저, 오늘 아침에 우리 집에서 대추를 땄다. 낼 제사 지내려고 …….”
    
    대추 한 줌을 내준다. 소년은 주춤한다.
    
    ::소녀::“맛봐라. 우리 증조(曾祖)할아버지가 심었다는데, 아주 달다.”
    
    소년은 두 손을 오그려 내밀며, 
    
    ::소년::“참, 알도 굵다!”
    
    ::소녀::“그리고 저, 우리 이번에 제사 지내고 나서 좀 있다. 집을 내주 게 됐다.”
    ...
    ::소녀::“왜 그런지 난 이사 가는 게 싫어졌다. 어른들이 하는 일이니 어쩔 수 없지만…….”
    ...
    낮에 봐 두었던 나무로 올라갔다. 그리고, 봐 두었던 가지를 향해 작대기를 내리쳤다. 호두송이 떨어지는 소리가 별나게 크게 들렸다. 가슴이 선뜩했다. 그러나 다음 순간, 굵은 호두야 많이 떨어 져라, 많이 떨어져라, 저도 모를 힘에 이끌려 마구 작대기를 내리 치는 것이었다.
